...
/** Start a voice+camera session and wait for the mic stream to be live. */
async function startVoiceCameraSession(page: Page): Promise<void> {
  ...
  const modeCard = page.getByRole("button", { name: "Voice + camera interview" });
  await expect(async () => {
