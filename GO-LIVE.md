...
- [x] ~~**LAUNCH100 promo**~~ — **RETIRED 2026-08-17.** The 100% code was pre-applying at checkout for anyone who had ever followed a `?promo=LAUNCH100` link (the code persists in localStorage for 90 days). Free access is now granted per-user from the admin page instead (Users → complimentary access: `compPlan` plus/professional + `compUntil` end date — no card, no Stripe). To retire a promo fully: deactivate the promotion code in the Stripe dashboard (kills pre-apply AND manual entry) and remove `LAUNCH_PROMO_CODE` from Vercel + redeploy (hides the pricing banner).
