...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    // Update name fields in Clerk (only if provided)
    ...
    // Update accountType in privateMetadata (merge — preserve other keys)
    if (body.accountType !== undefined) {
      ...
      await admin.client.users.updateUserMetadata(id, {
        privateMetadata: { ...existing, accountType: body.accountType },
      });
    }
