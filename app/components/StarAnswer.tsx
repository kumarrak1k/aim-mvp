  ...
  if (tone === "print") {
    return (
      <div className="space-y-3">
        {SECTIONS.map((s) => (
          <div key={s.key}>
            <p className="mb-0.5 text-[11px] font-black uppercase tracking-[0.14em] text-gray-500">
              {s.label}
            </p>
    ...
    <div className="space-y-3">
      {SECTIONS.map((s) => (
        <div key={s.key} className="flex items-start gap-3">
          <span
            aria-hidden
            className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br ${s.darkBadge} text-[11px] font-black text-white`}
          >
            {s.letter}
          </span>
          <div className="min-w-0 flex-1">
            <p className={`text-[11px] font-black uppercase tracking-[0.16em] ${s.darkText}`}>
              {s.label}
            </p>
